...
class ChipsControllerTest {
	...
	void setUp() throws Exception {
		...
		cc.getChips().addAll(IntStream.range(1000, 1050)
				.mapToObj(i -> {
					...
					// es muss noch die Runde -1 hinzugef�gt werden, da sonst 
					// zun�chst keine Doppelscan-Abfrage stattfindet 
	...
	void test() {
		...
		// 10 Sekunden warten, dann Chip einf�gen
		Thread t = new Thread(() -> {
			try {
				Thread.sleep(10000);
			} catch (InterruptedException e) {
				fail("Thread wurde gest�rt.");
			}
		});
		t.start(); 
		try {
			...
			fail("join wurde gest�rt.");
		}
		
		assertTrue(cc.addLap("1000") == 0);
		// lapCount muss jetzt 1 sein
		assertTrue(cc.getChipById("1000").getLapCount() == 1, "Runden werden falsch gez�hlt.");
	}
