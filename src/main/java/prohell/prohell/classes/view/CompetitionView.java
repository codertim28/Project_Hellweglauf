...
//	}
	...
	protected void setStartRounds() {
		...
		for(int i = 0; i < size; i++) {
			...
			// Die Startrunde soll nur eingef�gt werden, wenn der Sch�ler
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		//       -> Alle Runden l�schen und neu eintragen (es ist nur Runde -1 vorhanden)
		...
		if(comp.getState() == CompetitionState.PREPARE || comp.getState() == CompetitionState.READY) {
			...
			comp.getData().clear();
			// Jetzt in Ruhe zur�cksetzen
			for(final Chip c : chipsController.getChips()) {
				// Dies f�gt Runde -1 ein.
		...
		// Die Daten aus dem Wettkampf mit der Tabelle verkn�pfen.
	...
	/**
	 * Pr�ft die Bedingung und verändert diese, wenn der Benutzer dies fordert 
