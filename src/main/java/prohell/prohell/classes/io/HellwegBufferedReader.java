...
public class HellwegBufferedReader extends BufferedReader {
	...
	public Chip readChip() throws IOException {
		...
		while((line = readLine()) != null) {
			...
			// Pr�fen, wo sich der Reader befindet. 
			// - innerhalb eines Chips
			// - in der Rundenliste (<laps> ... </laps)
			if(line.indexOf("<chip>") != -1) {
				// true setzen, damit im n�chsten Durchlauf in die
				// die entsprechende Abfrage gesprungen wird.
				inChipTag = true;
			}
			...
			if(inChipTag) {
				...
				// Diese (und die n�chste) Abfrage ist an dieser Stelle nicht n�tig.
				// Der Quelltext wird auch ohne diese Abfrage ordnungsgem�� arbeiten.
			    ...
			    // Dann k�nnte sich diese Abrage als hilfreich erweisen.
				if(inLapsTag) {
